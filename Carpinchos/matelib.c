...
int crearConexion(char* ip, char* puerto) {
	...
	if (connect(clientSocket, server_info->ai_addr, server_info->ai_addrlen)
			== -1) {
...
void obtenerInfoConfigFinal(t_config* config, mate_instanceACambiar* carpincho) {
	...
	int tamIpKernel = string_length(info.ipKernel);
	int tamPuertoKernel = string_length(info.puertoKernel);
	int tamIpMemoria = string_length(info.ipMemoria);
	int tamPuertoMemoria = string_length(info.puertoMemoria);
	int tamAConectar = string_length(info.moduloAConectar);

	carpincho->info.ipKernel = string_substring(info.ipKernel, 0, tamIpKernel);
	carpincho->info.puertoKernel = string_substring(info.puertoKernel, 0, tamPuertoKernel);
	carpincho->info.ipMemoria = string_substring(info.ipMemoria, 0, tamIpMemoria);
	carpincho->info.puertoMemoria = string_substring(info.puertoMemoria, 0, tamPuertoMemoria);
	carpincho->info.moduloAConectar = string_substring(info.moduloAConectar, 0, tamAConectar);
...
void* serializarSemaforo(mate_sem_name semaforo) {
	int largoNombreSem = string_length(semaforo) + 1;
...
int mate_init(mate_instance *lib_ref, char *config) {
	...
	obtenerInfoConfigFinal(conf, carpincho);
...
int mate_close(mate_instance* lib_ref) {
	...
	if (string_equals_ignore_case(carpincho->info.moduloAConectar, "KERNEL") && carpincho->id > 0) {
		...
		close(carpincho->conexionMemoria);
...
//		//Recivo carpincho modificado
//		int sizeCarpiNuevo;
//		recv(conexion, &sizeCarpiNuevo, sizeof(int), MSG_WAITALL);
//		void* bufferRecv = malloc(sizeCarpiNuevo);
//		recv(conexion, bufferRecv, sizeCarpiNuevo, 0);
//
//		mate_instanceACambiar* nuevoCarpi = desserializarCarpincho(bufferRecv);
//		lib_ref->group_info = nuevoCarpi;
	...
	recv(carpincho->conexionMemoria, &direccion_alloc, sizeof(mate_pointer), MSG_WAITALL);
	}
...
int mate_memfree(mate_instance *lib_ref, mate_pointer addr) {
	...
	if (string_equals_ignore_case(carpincho->info.moduloAConectar, "KERNEL")) {
		...
		recv(carpincho->conexionMemoria, &respuesta, sizeof(int), MSG_WAITALL);
	}
...
int mate_memread(mate_instance* lib_ref, mate_pointer origin, void* dest, int size){ //size es el tamanio de dest
	...
	if (string_equals_ignore_case(carpincho->info.moduloAConectar, "KERNEL")) {
		...
		recv(carpincho->conexionMemoria, dest, size, MSG_WAITALL);
	}
