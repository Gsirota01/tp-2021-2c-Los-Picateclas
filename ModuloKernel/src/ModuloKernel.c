...
int main(void) {
	...
	int server_fd = iniciarServidor(info.ip, info.puertoEscucha, logKernel);
	int cliente_fd = esperar_cliente(server_fd, logKernel);

	while (1) {

		nuevoHilo(cliente_fd);

		cliente_fd = esperar_cliente(server_fd, logKernel);
	}
...
void procesarPeticion(arg_struct* argumentos) {
	...
	if (peticion == NUEVOPROCESO) {
		...
		int conexionMemoria = crearConexion(info.ipMemoria, info.puertoMemoria,logKernel);
...
int iniciarServidor(char* ip, char* puerto, t_log * log) {
	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		if ((socketServidor = socket(p->ai_family, p->ai_socktype,
				p->ai_protocol)) == -1)
			continue;

		int activado = 1;
		setsockopt(socketServidor, SOL_SOCKET, SO_REUSEADDR, &activado,
				sizeof(activado));
