...
int main(void) {
	...
	configKernel = config_create("/home/utnso/tp-2021-2c-Los-Picateclas/EntregaFinal/ModuloKernel/kernel.config");
	...
	int server_fd = iniciarServidor(info.puertoEscucha);
...
void cargarNuevoProceso(mate_instanceACambiar* proceso) {
	...
	if(queue_size(suspendReady) == 0 && queue_size(new) == 1){
		printf("VOY A HACER POST A READY\n");
